...
def test_successor_identity_does_not_promote_predecessor_permit(tmp_path, monkeypatch):
    assert gate.BATCH == "20260913-internal-nozzle-aux-projection-state-provenance-r1"
    f = fixture(tmp_path, monkeypatch)
    assert gate.scoped(f["run"])
    wrong = copy.deepcopy(f["contract"])
    for predecessor in ("20260912-internal-nozzle-restart-state-closure-r1", "20260912-internal-nozzle-pressure-repeatability-restart-closure-r1"):
        wrong["batch_identity"]["batch_id"] = predecessor
...
def test_start_ledger_fail_closed(tmp_path, monkeypatch, defect):
    ...
    if defect == "full_budget":
        rows = [{**row, "record_sha256": "a" * 64, "segment_id": "past-" + str(i)} for i in range(8)]
