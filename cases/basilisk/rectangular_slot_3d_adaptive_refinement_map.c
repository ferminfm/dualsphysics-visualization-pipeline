/**
# Rectangular-slot 3D adaptive refinement and sensitivity map

Bounded Basilisk VOF case for compact 3D sensitivity tests around the positive
2D `We_g = 80` scout and the negative 3D micro-translation. The focus is cost
control, slot resolution, and conservative breakup-proxy diagnostics.

This is a physics-route decision case only: not validation, not production CFD,
...
double slot_h = 0.8;
double dh = 0.96;
double duct_length_Dh = 1.5;
double external_length_Dh = 2.0;
double duct_length = 1.44;
double external_length = 1.92;
double domain_length = 3.36;
...
double refine_xmin_Dh = -0.75;
double refine_xmax_Dh = 1.25;
double refine_y_factor = 0.95;
double refine_z_factor = 1.10;
...
int main(int argc, char * argv[])
{
  ...
  if (argc > 17)
    refine_xmin_Dh = atof(argv[17]);
  if (argc > 18)
    refine_xmax_Dh = atof(argv[18]);
  if (argc > 19)
    refine_y_factor = atof(argv[19]);
  if (argc > 20)
    refine_z_factor = atof(argv[20]);
  if (argc > 21)
    tag_threshold = atof(argv[21]);
  ...
  if (target_we_g <= 0.)
    target_we_g = 80.;
  if (external_length_Dh < 1.)
    external_length_Dh = 1.;
  if (duct_length_Dh < 1.)
    duct_length_Dh = 1.;
  if (perturb_period <= 0.)
    perturb_period = 0.2;
  ...
  if (refine_y_factor < 0.5)
    refine_y_factor = 0.5;
  if (refine_z_factor < 0.5)
    refine_z_factor = 0.5;
  if (tag_threshold <= 0.)
    tag_threshold = 1e-3;
...
event init(t = 0)
{
  ...
  refine(x > refine_xmin_Dh*dh &&
         x < refine_xmax_Dh*dh &&
         fabs(y) < refine_y_factor*slot_w &&
         fabs(z) < refine_z_factor*slot_h &&
         level < pre_level);
...
event logfile(i++)
{
  if (i == 0) {
    ...
    fprintf(stderr,
            "design model 3D_rectangular_slot_adaptive_map Dh %g slot_w %g slot_h %g duct_Dh %g external_Dh %g domain_Dh %g We_g %g We_l %g rho_ratio_l_g %g mu_ratio_l_g %g Re_l %g Re_g %g Oh_l %g maxlevel %d minlevel %d slot_cells_w %g slot_cells_h %g perturb_amp %g perturb_period %g waves_y %g waves_z %g gas_mode %d gas_speed %g rel_u %g sigma %g pre_refine_offset %d refine_xmin_Dh %g refine_xmax_Dh %g refine_y_factor %g refine_z_factor %g tag_threshold %g\n",
            dh, slot_w, slot_h, duct_length/dh, external_length/dh,
            domain_length/dh, we_g, we_l, rho_l/rho_g, mu_l/mu_g,
            re_l, re_g, oh_l, maxlevel, minlevel, slot_w/delta, slot_h/delta,
            perturb_amp, perturb_period, perturb_waves_y, perturb_waves_z,
            gas_mode, gas_speed, rel_u, sigma0, pre_refine_level_offset,
            refine_xmin_Dh, refine_xmax_Dh, refine_y_factor, refine_z_factor,
            tag_threshold);
...
event outputs(t = 0.; t += output_interval; t <= end_time + 1e-12)
{
  ...
  view(camera = "iso", fov = 18.0, tx = -0.18, ty = 0.18,
