/**
# Rectangular-slot morphology escalation VOF case

Bounded Basilisk VOF case for escalating the rectangular-slot gas-Weber scan
after a negative `We_g = 50` result. The purpose is to test whether higher
slot resolution, longer near-exit evolution, stronger gas-side Weber forcing,
or a controlled gas-shear proxy can produce ligament, roll-up, or detached
liquid structures.

This is not validation, not production CFD, not stationary spray data, not
experimental agreement, and not a final atomisation prediction. Connected
...
double slot_h = 0.8;
double dh = 0.96;
double duct_length_Dh = 10.;
double external_length_Dh = 7.;
double duct_length = 9.6;
double external_length = 6.72;
double domain_length = 16.32;
...
event logfile(i++)
{
  if (i == 0) {
    ...
    fprintf(stderr,
            "design Dh %g duct_Dh %g external_Dh %g domain_Dh %g We_g %g We_l %g rho_ratio_l_g %g mu_ratio_l_g %g Re_l %g Re_g %g Oh_l %g maxlevel %d slot_cells_w %g slot_cells_h %g perturb_amp %g perturb_period %g waves_y %g waves_z %g gas_mode %d gas_speed %g rel_u %g sigma %g\n",
            dh, duct_length/dh, external_length/dh, domain_length/dh,
            we_g, we_l, rho_l/rho_g, mu_l/mu_g, re_l, re_g, oh_l,
            maxlevel, slot_w/delta, slot_h/delta, perturb_amp,
            perturb_period, perturb_waves_y, perturb_waves_z,
...
event outputs(t = 0.; t += output_interval; t <= end_time + 1e-12)
{
  ...
  view(camera = "iso", fov = 13.5, tx = -0.38, ty = 0.28,
...
event adapt(i++)
{
  ...
  boundary({speed});
  adapt_wavelet({f,u,speed}, {0.004,uemax,uemax,uemax,0.04}, maxlevel);
}
