...
- `docs/basilisk_rectangular_atomisation_blender_showcase.md`
...
**Official-style Basilisk wrapper:** A bounded wrapper around the local
Basilisk `examples/atomisation.c` structure is available at
`cases/basilisk/basilisk_official_atomisation_bounded.c`. The 2026-06-18 medium
run produced native `draw_vof` frames and `tag()` component diagnostics under
`/home/franco/stack-validation/20260618-basilisk-official-atomisation-wrapper`.
It is internal technical evidence for the liquid-gas interface route, not
public validation or stationary spray data.

**Rectangular-slot gas-Weber scan:** A follow-up VOF case at
`cases/basilisk/rectangular_slot_gas_weber_proxy.c` adds a short rectangular
duct, a gas-side Weber target (`We_g = 50`), and a reproducible transverse
perturbation. The bounded 2026-06-18 runs produced native `draw_vof` frames,
diagnostic projection media, and CSV/JSON component and slice metrics under
`/home/franco/stack-validation/20260618-basilisk-rectangular-slot-gas-weber-scan`.
The observed morphology was connected waviness/bulging, not detached-volume
breakup and not atomization. See `docs/basilisk_atomisation_route.md`.

**Morphology escalation scan:** A parametric follow-up case at
`cases/basilisk/rectangular_slot_morphology_escalation.c` tested higher slot
resolution, `We_g = 100-150`, stronger perturbation, and a controlled gas
crossflow proxy. The completed ladder remained negative: no case produced more
than one credible post-exit connected component, and no detached-volume proxy
was detected. The result is internal physics-route evidence, not a public
atomisation artifact.

**2D/planar shear-sigma scout:** A lower-cost reduced-model follow-up at
`cases/basilisk/planar_slot_shear_sigma_scout.c` found a scout candidate at
`We_g = 80`: native VOF frames show rolled-up sheet edges and repeated
post-exit detached-component proxies, including a maxlevel-11 repeat. This is a
parameter-scout result for later 3D translation, not 3D validation, stationary
spray data, or final atomisation prediction. See
`docs/basilisk_atomisation_route.md`.

**3D micro-translation of the We_g = 80 scout:** A compact 3D follow-up at
`cases/basilisk/rectangular_slot_3d_micro_translation_we80.c` tested whether the
positive 2D mild-perturbation window transfers back to a rectangular-slot VOF
case. Under the bounded 2026-06-18 runs, maxlevel-9 attempts were
resource-limited before a useful morphology window, and the affordable
maxlevel-8 cases remained one connected post-exit component with detached proxy
count `0`. This is a negative transfer result for the tested constraints, not
atomization evidence. See `docs/basilisk_atomisation_route.md`.

**3D adaptive refinement map:** A follow-up case at
`cases/basilisk/rectangular_slot_3d_adaptive_refinement_map.c` tested shorter
domains, explicit refinement controls, mild sigma/perturbation changes, and a
minimal crossflow proxy. The high-resolution branch stayed cost-limited, and
